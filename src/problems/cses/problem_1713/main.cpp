...
// Using a fixed-size array instead of std::vector
std::array<int, MAX_N + 1> sieve() {
  std::array<int, MAX_N + 1> max_div = {};  // Initialize with zeros
  for (int i = 2; i <= MAX_N; ++i) {
    if (max_div[i] == 0) {  // i is prime
      for (int j = i; j <= MAX_N; j += i) {
        if (max_div[j] == 0) {
          max_div[j] = i;  // The smallest prime factor
        }
      }
    }
  }
...
void solve() {
  ...
  ll result{1};
  while (n != 1) {
    const ll prime_divisor = MAX_DIV[n];
    ll count{0};
