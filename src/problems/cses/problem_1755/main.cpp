...
void solve() {
  ...
  for (auto [c, f] : freq) {
    if (f % 2 != 0) {
      odd_count++;
      center = string(1, c);  // Set the center character
    }
  }
  ...
  for (auto [c, f] : freq) {
    const string part(f / 2, c);  // Get half of occurrences
    left_half += part;
  }

  string right_half = left_half;
  ranges::reverse(right_half);

  cout << left_half + center + right_half << endl;
}
