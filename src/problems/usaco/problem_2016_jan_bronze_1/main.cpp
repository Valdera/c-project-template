...
void solve() {
  ll before_bronze, after_bronze, before_silver, after_silver, before_gold,
      after_gold, before_platinum, after_platinum;
  ...
  ll total = 0;
  vi diffs = {after_platinum - before_platinum, after_gold - before_gold,
              after_silver - before_silver};
  vi results = {0, 0, 0};
  for (int i = 0; i < 3; i++) {
    total += diffs[i];
    results[3 - i - 1] = total;
