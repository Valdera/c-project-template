...
void solve() {
  ...
  map<int, int> positionToDistance;

  for (int i = 0; i < n; i++) {
    int x, d;
    cin >> x >> d;
    camels[i] = {x, d};
    positionToDistance[x] = d;
  }

  for (int i = 0; i < n; i++) {
    int x = camels[i].first;
    int d = camels[i].second;
    int target = x + d;

    // Check if there's a camel at position target that spits back at x
    if (positionToDistance.count(target) && positionToDistance[target] == -d) {
      cout << "YES" << endl;
      return;
    }
  }
