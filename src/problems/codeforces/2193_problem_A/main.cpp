...
void solve() {
  ...
  int diff = s - current_sum;

  // If difference is negative, we can't reduce the sum (we can only add)
  if (diff < 0) {
    cout << "NO\n";
    return;
  }

  // If difference is 0, already equal
  if (diff == 0) {
    cout << "YES\n";
    return;
  }

  // If difference is positive, check if it's divisible by x
  if (diff % x == 0) {
    cout << "YES\n";
  } else {
    cout << "NO\n";
