...
void solve() {
  ...
  // For each subsequent string, choose to add to beginning or end
  for (int i = 1; i < n; i++) {
    string add_to_beginning = a[i] + s;
    string add_to_end = s + a[i];

    // Choose lexicographically smaller option
    if (add_to_beginning < add_to_end) {
      s = add_to_beginning;
    } else {
      s = add_to_end;
    }
