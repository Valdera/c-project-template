...
void solve() {
    ...
    // Let h = chickens, c = cows
    // Equation: 2h + 4c = n
    // Simplify: h + 2c = n/2
    // So: h = n/2 - 2c

    // For valid configurations:
    // 1. n must be even (otherwise no integer solution)
    // 2. h >= 0: n/2 - 2c >= 0, so c <= n/4
    // 3. c >= 0

    // If n is odd, no configurations possible
    if (n % 2 == 1) {
        cout << 0 << "\n";
        return;
    }

    // If n is even, c can range from 0 to floor(n/4)
    // Number of configurations = floor(n/4) + 1
    int configurations = n / 4 + 1;
    cout << configurations << "\n";
}
