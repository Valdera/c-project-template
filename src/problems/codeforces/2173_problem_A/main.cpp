...
void solve() {
    ...
    // Track which classes we must stay awake for
    vector<bool> awake(n, false);

    // For each important class, mark it and the next k classes as awake
    for (int i = 0; i < n; i++) {
        if (s[i] == '1') {
            // Mark classes i, i+1, ..., i+k as awake (or until end of day)
    ...
    // Count classes we can sleep through
    int sleep_count = 0;
    for (int i = 0; i < n; i++) {
        if (!awake[i]) {
            sleep_count++;
