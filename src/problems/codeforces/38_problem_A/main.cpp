...
void solve() {
    ...
    // To go from rank a to rank b, we need to sum:
    // d[a-1] + d[a] + ... + d[b-2] (0-indexed)
    // which is d[a] + d[a+1] + ... + d[b-1] (1-indexed)

    int total_years = 0;
    for (int i = a; i < b; i++) {
        total_years += d[i - 1]; // d is 0-indexed, rank i is at index i-1
    }
